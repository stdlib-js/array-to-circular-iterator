...
function circarray2iterator( src ) {
	...
	var i;
	if ( !isCollection( src ) ) {
		throw new TypeError( format( '01V2O', src ) );
	}
	opts = {
		'iter': 1e308, // ~infinity
	...
	if ( arguments.length > 1 ) {
		if ( isObject( arguments[ 1 ] ) ) {
			...
			if ( arguments.length > 2 ) {
				fcn = arguments[ 2 ];
				if ( !isFunction( fcn ) ) {
					throw new TypeError( format( '01V2b', fcn ) );
				}
				thisArg = arguments[ 3 ];
			}
			if ( hasOwnProp( options, 'iter' ) ) {
				...
				if ( !isNonNegativeInteger( options.iter ) ) {
					throw new TypeError( format( '01V2t', 'iter', options.iter ) );
				}
			}
			if ( hasOwnProp( options, 'dir' ) ) {
				opts.dir = options.dir;
				if ( options.dir !== 1 && options.dir !== -1 ) {
					throw new TypeError( format( '01V2v', 'dir', options.dir ) );
				}
			}
		} else {
			fcn = arguments[ 1 ];
			if ( !isFunction( fcn ) ) {
				throw new TypeError( format( '01V2w', fcn ) );
			}
			thisArg = arguments[ 2 ];
		}
	}
